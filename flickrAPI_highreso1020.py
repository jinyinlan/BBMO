...
class Flickr(object):
	...
	@retry(tries=4, delay=5, backoff=2)
	def check_license(self, photo_id):
		r = requests.post(self.api_url, {'api_key': self.api_key,
		...
		dom = md.parseString(r.text.encode('utf-8'))
		result = False
		for elem in dom.getElementsByTagName('license'):
			# smallのサイズのもののみにする
			if elem.getAttribute('name') == 'All Rights Reserved':
				result = True
				print('All Rights Reserved')
			elif elem.getAttribute('name') == 'Attribution-NonCommercial-ShareAlike License':
				print('Attribution-NonCommercial-ShareAlike License')
			elif elem.getAttribute('name') == 'Attribution-NonCommercial License':
				print('Attribution-NonCommercial License')
			elif elem.getAttribute('name') == 'Attribution-NonCommercial-NoDerivs License':
				result = True
				print('Attribution-NonCommercial-NoDerivs License')
			elif elem.getAttribute('name') == 'Attribution License':
				print('Attribution License')
			elif elem.getAttribute('name') == 'Attribution-ShareAlike License':
				print('Attribution-NoDerivs License')
			elif elem.getAttribute('name') == 'Attribution-NoDerivs License':
				result = True
				print('Attribution-NoDerivs License')
			elif elem.getAttribute('name') == 'No known copyright restrictions':
				print('No known copyright restrictions')
			elif elem.getAttribute('name') == 'Public Domain Dedication (CC0)':
				print('Public Domain Dedication (CC0)')
			elif elem.getAttribute('name') == 'Public Domain Mark':
				print('Public Domain Mark')
...
def download_image(photo_url, photo_id, photo_date):
	savedir = '/home/omiya/data/test-set1020/'+photo_date+'/'
	if not os.path.exists(savedir):
		os.mkdir(savedir)
...
if __name__ == '__main__':
	...
	for i in range(size):
		if not(f.check_tags(id_list[i])):
			...
			if not(f.check_license(id_list[i])):
				if (url_list[i]):
					download_image(url_list[i], id_list[i], photo_date)
					all_photos_name.append(photo_date+'-'+id_list[i])
					print('   ', id_list[i], ': is saved.')	
				else:#url_list=None
					print(id_list[i], ': url is None.')	
					time.sleep(0.5)
			else: 
				print(id_list[i], 'license: not saved')
		else:
			print(id_list[i], 'blackwhite: not saved')
		

	with open( '/home/omiya/data/test-set1020/'+photo_date+'/all_file_name.txt', 'w' ) as file:
